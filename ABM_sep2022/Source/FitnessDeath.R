...
FitnessDeath = function(pop, maturity, y, purge, nSNP.cons){
  ...
  print(paste(nrow(pop), nrow(mature), nrow(npurged), nrow(dead))) #nrow(nkilled), 
  
  pop = rbind(pop, mature, npurged, dead)  #combine pop, mature, and previously removed dead indv #nkilled, 
  
  remove(dead, mature, npurged)   #clean up #, nkilled
